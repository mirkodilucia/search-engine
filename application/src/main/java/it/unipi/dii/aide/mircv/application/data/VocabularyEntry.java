...
public class VocabularyEntry
{
    ...
    public void debugSaveToDisk(String path) {
        ...
        FileUtils.createFile("data/debug/" + path);
    ...
    public void updateStatistics(PostingList intermediatePostingList) {
        for (Posting pos : intermediatePostingList.getPostings()) {
            if (pos.getFrequency() > this.maxTermFrequency) {
                this.maxTermFrequency = pos.getFrequency();
            }
            this.documentFrequency++;
    ...
    public void updateBM25Statistics(int termFrequency, int documentLength) {
        double currentRatio = (double) this.BM25Tf / (double) (this.BM25Dl + this.BM25Tf);
        double newRatio = (double) termFrequency / (double) (documentLength + termFrequency);
        if(newRatio > currentRatio){
            this.BM25Tf = termFrequency;
            this.BM25Dl = documentLength;
        }
    }
}
